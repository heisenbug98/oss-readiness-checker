...
The v0.3.0 release added public GitHub repository scanning so a maintainer can inspect a repo by `owner/repo` or GitHub URL, not only from a local checkout.
...
```text
OSS Readiness Checker is a maintained CLI I built to help public repos surface maintainer readiness signals: docs, license, security policy, templates, tests, CI, changelog, releases, and public GitHub repo scans. It has issue->PR->CI->merge->release workflow through v0.3.0, 100/100 self-checks, tests for API failures, public docs, and an external OSS PR with all checks passing.
```
